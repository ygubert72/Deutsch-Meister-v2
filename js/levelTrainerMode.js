...
// ПРАВИЛЬНЫХ СЛОВ МОЖЕТ БЫТЬ 1-4 (НО НЕ БОЛЬШЕ 4)
// ====================================================================
...
function ensureSixButtonsWithCorrectWords() {
    ...
    while (correctCount > maxCorrect) {
        ...
        if (extraCorrectIndex !== -1) {
            const removed = levelTrainerAvailableWords.splice(extraCorrectIndex, 1)[0];
            _wordQueue.push({
                id: ++_wordIdCounter,
    ...
    while (levelTrainerAvailableWords.length < maxButtons) {
    ...
    while (levelTrainerAvailableWords.length > maxButtons) {
        ...
        if (distractorIdx !== -1) {
            levelTrainerAvailableWords.splice(distractorIdx, 1);
        } else {
...
function createInitialButtons() {
    ...
    console.log('🔍 createInitialButtons: направление =', levelTrainerDirection);
    ...
    _cachedDistractors = allDistractorWords;
    ...
    while (levelTrainerAvailableWords.length < 6 && distractorIndex < filteredDistractors.length) {
        ...
        const exists = levelTrainerAvailableWords.some(w => w.display === d.display);
        if (!exists) {
            levelTrainerAvailableWords.push({
                id: ++_wordIdCounter,
                display: d.display,
                de: d.de,
                ru: d.ru,
    ...
    console.log('   Направление:', levelTrainerDirection);
...
function returnAllWordsToButtons() {
    ...
    if (isRuToDe) {
        ...
        firstThreeCorrect = allCorrect.slice(0, 4);
    ...
    let distractorIndex = 0;
    while (levelTrainerAvailableWords.length < 6 && distractorIndex < onlyDistractors.length) {
...
function attachLevelTrainerEvents() {
    ...
    if (dirBtn) {
        dirBtn.addEventListener('click', function() {
            ...
            console.log('🔄 Направление изменено на:', levelTrainerDirection);
            console.log('🔄 Кеш дистракторов сброшен');
    ...
    if (wordsContainer) {
        wordsContainer.addEventListener('click', function(e) {
            ...
            if (correctOnButtons < 4 && _wordQueue.length > 0) {
                ...
                if (distractorIndex !== -1) {
                    ...
                    insertWordAtRandomPosition(levelTrainerAvailableWords, newWord);
                } else {
...
console.log('🧩 levelTrainerMode.js загружен (макс. 4 правильных слова)');
