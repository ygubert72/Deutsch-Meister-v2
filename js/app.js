...
// ================================================================
// НОВЫЙ КОД: режимы Карточки и Фразы (выбор режимов)
// ================================================================

// ========== ПОКАЗ ВЫБОРА РЕЖИМА ==========
async function showLevelModeSelector(level) {
    if (typeof window.currentLevel !== 'undefined') {
        window.currentLevel = level;
    }
    if (typeof window.currentMode !== 'undefined') {
        window.currentMode = 'levelSelect';
    }
    ...
    if (indicator) indicator.textContent = `📚 Уровень ${level}`;
    if (counter) counter.textContent = '';
    ...
    content.innerHTML = `
        <div style="max-width:800px; margin:0 auto; padding:20px 15px;">
            <h2 style="font-size:28px; margin:0 0 5px 0;">📚 Уровень ${level}</h2>
            <p style="color:#666; margin-bottom:25px;">Выберите режим обучения</p>
            
            <div style="display:grid; grid-template-columns: repeat(3, 1fr); gap:15px; margin-bottom:30px;">
                <button class="mode-select-btn" data-mode="lessons" style="
                    padding:20px 15px;
                    background:white;
                ...
                ">
                    ...
                    <div style="font-weight:bold; font-size:16px;">Уроки</div>
                    <div style="font-size:12px; color:#888;">${stats.lessons} уроков</div>
                </button>
                
                <button class="mode-select-btn" data-mode="cards" style="
                    padding:20px 15px;
                    background:white;
                    border:3px solid #E0E0E0;
                    border-radius:16px;
                ...
                ">
                    <div style="font-size:36px; margin-bottom:8px;">🃏</div>
                    <div style="font-weight:bold; font-size:16px;">Карточки</div>
                    <div style="font-size:12px; color:#888;">${stats.words} слов</div>
                </button>
                
                <button class="mode-select-btn" data-mode="phrases" style="
                    padding:20px 15px;
                    background:white;
                    border:3px solid #E0E0E0;
                    border-radius:16px;
                ...
                ">
                    ...
                    <div style="font-weight:bold; font-size:16px;">Фразы</div>
                    <div style="font-size:12px; color:#888;">${stats.phrases} фраз</div>
                </button>
            </div>
            
            <div id="modeContent">
                <p style="text-align:center; color:#999; padding:30px 0;">
                    👆 Выберите режим обучения выше
                </p>
            </div>
        </div>
    `;
        ...
        btn.addEventListener('click', function() {
            ...
            if (mode === 'lessons') {
                ...
                if (typeof window.renderCards === 'function') {
                    window.renderCards(container, level);
                } else {
                    container.innerHTML = '<div style="text-align:center;padding:40px;color:#999;">⏳ Режим Карточки загружается...</div>';
                }
            } else if (mode === 'phrases') {
                if (typeof window.renderPhrases === 'function') {
                    window.renderPhrases(container, level);
                } else {
                    container.innerHTML = '<div style="text-align:center;padding:40px;color:#999;">⏳ Режим Фразы загружается...</div>';
                }
            }
        });
        
        btn.addEventListener('mouseenter', function() {
            if (this.style.borderColor !== '#3B6FE0') {
...
// ========== ЗАГРУЗКА СТАТИСТИКИ УРОВНЯ ==========
async function loadLevelStats(level) {
    try {
        const indexResponse = await fetch(`docs/${level}/index.json`);
        const indexData = await indexResponse.json();
        const lessons = indexData.lessons ? indexData.lessons.length : 0;
        
        let words = 0;
        try {
            const wordsResponse = await fetch(`docs/${level}.json`);
            if (wordsResponse.ok) {
        ...
        return { lessons, words, phrases };
    } catch(e) {
        return { lessons: 0, words: 0, phrases: 0 };
    }
}
